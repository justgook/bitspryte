...
- `UI_GRAPHICS_SCALE` selects a positive integer nearest-neighbor scale for the fixed SMGUI skin, named Panel style atlas, and right-toolbar icons. Font scale and general application geometry remain independent.
- The right tool sidebar remains 32px wide. Its tool buttons preserve explicit 32×32 geometry with no horizontal inset, 3px top inset, and 1px vertical gaps; buttons are top-aligned vertically and centered horizontally even when imported icon bounds use a larger `UI_GRAPHICS_SCALE`; transparent icon margins may extend beyond intrinsic content measurement. Tool selection is an exclusive integer binding. Toolbar Buttons override SMGUI's fixed skin with the independently replaceable `buttonset_item_focused`/`buttonset_item_hot` named nine-slice pair; hover and selected/pressed states choose the pair's focused slice.
